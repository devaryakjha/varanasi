...
func createOverviewPage(data models.ArtistDetailsRequest) models.Page {
	...
	for i := 0; i < modulesLen; i++ {
		...
		if dataSourceKey == "featured_artist_playlist" {
			blocks = append(blocks, models.Block{
				Sequence:    3,
				Title:       mod.Title,
				Orientation: orientation,
				Children:    createMediaListFromFeaturedArtistPlaylist(data),
			})
			continue
		}

		if dataSourceKey == "topAlbums" {
			blocks = append(blocks, models.Block{
				Sequence:    4,
				Title:       "Top Albums",
				Orientation: orientation,
				Children:    extensions.Map(data.TopAlbums, utils.CreateMediaFromAlbumRequest, 0),
			})
			continue
		}

		if dataSourceKey == "singles" {
			blocks = append(blocks, models.Block{
				Sequence:    5,
				Title:       mod.Title,
				Orientation: orientation,
				Children:    extensions.Map(data.Singles, utils.CreateMediaFromAlbumRequest, 0),
			})
			continue
		}

		if dataSourceKey == "latest_release" {
			blocks = append(blocks, models.Block{
				Sequence:    6,
				Title:       mod.Title,
				Orientation: orientation,
				Children:    extensions.Map(data.LatestRelease, utils.CreateMediaFromAlbumRequest, 0),
			})
			continue
		}
...
func createMediaListFromFeaturedArtistPlaylist(data models.ArtistDetailsRequest) []models.Media {
	var playlist = data.FeaturedArtistPlaylist
	result := extensions.Map(playlist, func(item models.FeaturedArtistPlaylistItem) models.Media {
		return models.Media{
			ID:       item.ID,
			Title:    item.Title,
